...
首先要知道<u>JS是动态解释脚本语言，也就是会一行一行解释，但进行解释前JS引擎会对代码进行语法分析</u>，也就是变量声明等一系列的处理，如下面一段代码，这是一段很普通的定义变量的代码块，结果已经标识出来了，看和你想的一致不。首先<u>语法分析时，JS引擎会扫描当前(这里指全局)作用域的变量</u>，如上name、age然后定义他们，但并没有赋值也就是值为`undefined`，所以如果当前作用域一个变量没有定义前去访问它并不会报错，而是打印出`undefined`(1,2,4行)也就是<u>未定义值的变量</u>。而赋值只会在JS引擎解释到当前赋值操作时才会真正赋值(6行)，然后在7行再次打印`name`时，就正确打印了name的值，因为此时name已经进行赋值了，而最后一行访问不存在的addr变量时则会报错。
```js{1,2,4,7}
...
![JS内存图](https://tva1.sinaimg.cn/large/005HV6Avgy1h7s1cgmyihj30kw08rmyg.jpg)
...
![Call Context](https://tva1.sinaimg.cn/large/005HV6Avgy1h7s1w3av44j30j8092773.jpg)
